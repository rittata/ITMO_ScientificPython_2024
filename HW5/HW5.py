...
# def desc_calc(data, mode='train', log=None): # fill the necessary arguments
#     return getAllDescriptors(data, mode, log)

os.chdir('/content/smdt')
...
def fit_Lasso(X_train, X_test, y_train, y_test):
    ...
    a = Imputer(missing_values=np.nan, strategy='median')#, axis=0)
    b = StandardScaler()
...
def sar_model_evaluation(descriptors: pd.DataFrame):
    ...
    X = train_descriptors.drop('Target', axis=1)
    y = train_descriptors['Target'].values
...
def sar_model_train(descriptors_train: pd.DataFrame, indices, model1):
    ...
    y_train = descriptors_train['Target'].values
...
MAIN PART
'''
train_data = pd.read_csv('/content/smdt/logp_full.csv')
pred_data = pd.read_csv('/content/smdt/logp_inputs.csv')
...
if __name__ == "__main__":
    ...
    # loading data
    train_data = pd.read_csv('/content/smdt/logp_100.csv')
    pred_data = pd.read_csv('/content/smdt/logp_inputs.csv')
    cpds = [row for row in pred_data.loc[:, 'SMILES']]

    # calculating descriptors
    print("Calculating descriptors for training data...")
    train_descriptors = getAllDescriptors(train_data, mode='train', log=None)
    print("Calculating descriptors for prediction data...")
    pred_descriptors = getAllDescriptors(pred_data, mode='pred', log=None)
    ...
    final_model = sar_model_train(train_descriptors, cols, model[0])
...
# Best parameters are: {'anova__k': 20}
# Training the model with the best parameters...
# Predicted LogP value for compound CC1=CC2=C(C=C1C)NC(=O)CC2C3=CC=CC=C3OC: [4.29173918]
# Searching for similar compunds...
# Filtering logP...
# Request for compound CC1=CC2=C(C=C1C)NC(=O)CC2C3=CC=CC=C3OC completed. I found the following CIDs in PubChem with XLogP in the range of [4.29173918]+- 10%: [(2906028, 4), (2917087, 4.5), (17285549, 4.1)]
# Predicted LogP value for compound COC1=CC=C2C(=C1)OC(CC2=O)(C(F)(F)F)O: [2.40392046]
# Searching for similar compunds...
# Filtering logP...
# Request for compound COC1=CC=C2C(=C1)OC(CC2=O)(C(F)(F)F)O completed. I found the following CIDs in PubChem with XLogP in the range of [2.40392046]+- 10%: [(2917555, 2.4), (2849054, 2.2), (799275, 2.4)]
